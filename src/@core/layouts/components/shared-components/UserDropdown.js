  ...
  useEffect(() => {
    ...
    const verifyTokenAndFetchUser = async () => {
      try {
        ...
        console.error(error)
        window.location.href = '/pages/login/' // Redirect to login if token is invalid or not found
      }
    }
        ...
        <Box sx={{ pt: 2, pb: 3, px: 4 }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            <Box sx={{ display: 'flex', marginLeft: 3, alignItems: 'flex-start', flexDirection: 'column' }}>
              <Typography sx={{ fontWeight: 600 }}>{userInfo.firstName + ' ' + userInfo.lastName}</Typography>
